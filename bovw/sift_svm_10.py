...
for count in range(1,11):
    ...
    for nor in norm:
        for n in k_range:
            ...
            centers = np.load("cancer\\sift\\normal\\"+ "sift_cv" + str(count) + "_k" + str(n) + ".npy")
            ...
            train_npy = np.load("cancer\\sift\\normal\\"+ "sift_cv" + str(count) + "_train" + ".npy")
            print("成功导入训练数据")
            label_npy = np.load("cancer\\sift\\normal\\"+ "sift_cv" + str(count) + "_label" +  ".npy")
            print("成功导入标签数据")
            print('begin train classfier')

            a_dict = label_npy[()]

            for f in range(len(a_dict)):
                im = train_npy[f]
                des = calcSiftFeature(im)
                ...
                labels = np.append(labels, np.float32(label_npy[f]))
            # dictIdx += 1
            ...
            test_data = np.load("cancer\\sift\\normal\\"+ "sift_cv" + str(count) + "_test" + ".npy")
            print("成功导入训练数据")
            test_label = np.load("cancer\\sift\\normal\\"+ "sift_cv" + str(count) + "_test_label" +  ".npy")
            print("成功导入标签数据")
            ...
            print(classification_report(test_label, testresults, target_names=target_names))
            f = open("D:\\592\\image_classification_using_sift\\cancer\\sift\\normal\\" + "cross" + str(count)+".txt",'a', encoding='utf-8')
            space = "********************************************************\n"
            mes = "sift" + "cross" + str(count) + " " + "k" + str(n) + " " + "norm: " + str(nor) + "\n"
            f.writelines(space)
            f.writelines(mes)
            f.writelines(classification_report(test_label, testresults, target_names=target_names))
            f.close()
